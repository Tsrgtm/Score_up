...
>
    ...
    <div class="hidden lg:flex gap-12 items-center">
        ...
        @auth
            <x-dropdown class="relative">
                <x-slot name="trigger">
                    <button class="text-slate-600 flex items-center hover:bg-gray-50 rounded-md shadow-none transition-all duration-300">
                        <x-avatar search="{{ auth()->user()->name }}" provider="gravatar" class="w-8 h-8 border rounded-md" />
                        <div class="flex items-center py-2 px-1">
                            <x-heroicon-o-chevron-down class="w-4 h-4" />
                        </div>
                    </button>
                </x-slot>
                <div class="absolute right-0 mt-2 w-max bg-amber-100 border border-gray-200 rounded-md overflow-hidden">
                    <a href="#" class="flex gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-amber-300 transition-all duration-300">
                    ...
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        <button type="submit" class="flex gap-3 px-4 py-2 w-full text-sm text-gray-700 hover:bg-red-500 hover:text-gray-100 transition-all duration-300 cursor-pointer">
    ...
    <div class="lg:hidden flex gap-4 font-medium">
        ...
        <div class="p-1 bg-amber-500 hover:bg-amber-600 text-white rounded-lg">
            <x-heroicon-s-bars-3 class="w-8 h-8" />
        </div>
    </div>
</header>
